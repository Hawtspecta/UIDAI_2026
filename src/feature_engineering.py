...
class UFIFeatureEngine:
    ...
    def calculate_demographic_update_intensity(self):
        ...
        demo_agg = (
            self.demo.groupby(["state", "district"])
        ...
        enrol_agg = (
            self.enrol.groupby(["state", "district"])
            .agg({"age_0_5": "sum", "age_5_17": "sum", "age_18_greater": "sum"})
            .reset_index()
        )

        enrol_agg["total_enrollments"] = (
            enrol_agg["age_0_5"] + enrol_agg["age_5_17"] + enrol_agg["age_18_greater"]
        )
        ...
        if before - after > 0:
            print(
                f"   🧹 Dropped {before - after} low-population districts (<{MIN_ENROLLMENTS} enrollments)"
            )
        ...
        print(
            f"   ✅ Range: {merged['demo_update_intensity'].min():.2f} - {merged['demo_update_intensity'].max():.2f}"
        )
        print(f"   ✅ Mean: {merged['demo_update_intensity'].mean():.2f}")

        return merged[
            ["state", "district", "demo_update_intensity", "total_enrollments"]
        ]
    ...
    def calculate_biometric_refresh_rate(self):
        ...
        self.bio["date"] = pd.to_datetime(
            self.bio["date"], format="%d-%m-%Y", errors="coerce"
        )
        self.bio["year_month"] = self.bio["date"].dt.to_period("M")

        # Calculate monthly updates by district
        bio_monthly = (
            self.bio.groupby(["state", "district", "year_month"])
        ...
        bio_monthly["bio_refresh_rate"] = (
            bio_monthly.groupby(["state", "district"])["total_bio_updates"].pct_change()
            * 100
        )

        # Aggregate to district level (mean growth rate)
        bio_rate = (
            bio_monthly.groupby(["state", "district"])["bio_refresh_rate"]
            .mean()
            .reset_index()
        )
        ...
        bio_rate["bio_refresh_rate"] = bio_rate["bio_refresh_rate"].replace(
            [np.inf, -np.inf], 0
        )

        print(
            f"   ✅ Range: {bio_rate['bio_refresh_rate'].min():.2f} - {bio_rate['bio_refresh_rate'].max():.2f}"
        )
    ...
    def calculate_age_group_disparity(self):
        ...
        merged = pd.merge(
            demo_age, enrol_age, on=["state", "district"], how="outer"
        ).fillna(0)
        ...
        merged["total_enrollments"] = merged[
        ...
        merged["age_disparity"] = abs(
            merged["young_update_rate"] - merged["elder_update_rate"]
        )

        print(
            f"   ✅ Range: {merged['age_disparity'].min():.2f} - {merged['age_disparity'].max():.2f}"
        )
    ...
    def calculate_update_enrollment_ratio(self):
        ...
        demo_total["total_demo"] = (
            demo_total["demo_age_5_17"] + demo_total["demo_age_17_"]
        )

        bio_total = (
            self.bio.groupby(["state", "district"])
            .agg({"bio_age_5_17": "sum", "bio_age_17_": "sum"})
            .reset_index()
        )
        ...
        print(
            f"   ✅ Range: {merged['update_enrol_ratio'].min():.2f} - {merged['update_enrol_ratio'].max():.2f}"
        )
    ...
    def calculate_temporal_volatility(self):
        ...
        self.demo["date"] = pd.to_datetime(
            self.demo["date"], format="%d-%m-%Y", errors="coerce"
        )
        self.demo["year_month"] = self.demo["date"].dt.to_period("M")

        demo_monthly = (
            self.demo.groupby(["state", "district", "year_month"])
            .agg({"demo_age_5_17": "sum", "demo_age_17_": "sum"})
            .reset_index()
        )
        demo_monthly["monthly_updates"] = (
            demo_monthly["demo_age_5_17"] + demo_monthly["demo_age_17_"]
        )

        # Calculate coefficient of variation by district
        volatility = (
            demo_monthly.groupby(["state", "district"])["monthly_updates"]
            .agg([("mean", "mean"), ("std", "std")])
            .reset_index()
        )

        volatility["temporal_volatility"] = np.where(
            volatility["mean"] > 0, (volatility["std"] / volatility["mean"]) * 100, 0
        )

        # Replace inf values
        volatility["temporal_volatility"] = volatility["temporal_volatility"].replace(
            [np.inf, -np.inf], 0
        )

        print(
            f"   ✅ Range: {volatility['temporal_volatility'].min():.2f} - {volatility['temporal_volatility'].max():.2f}"
        )
