...
class AadhaarDataLoader:
    ...
    def _find_file(self, substring: str, sample_size: str = None):
        ...
        if not self.data_dir.exists():
            raise FileNotFoundError(
                f"Data directory does not exist: {self.data_dir.resolve()}"
            )
        ...
        if not matches_list:
            ...
            contains_sub = [
                p for p in candidates if substring.lower() in p.name.lower()
            ]
            if contains_sub:
    ...
    def load_enrollment_data(
        self, sample_size: str = "500000", file_path: str = None
    ) -> pd.DataFrame:
        ...
        if file_path:
            ...
            if not file.exists():
                raise FileNotFoundError(
                    f"Provided enrollment file does not exist: {file}"
                )
        else:
    ...
    def load_biometric_data(
        ...
        if file_path:
            ...
            if not file.exists():
                raise FileNotFoundError(
                    f"Provided biometric file does not exist: {file}"
                )
        else:
    ...
    def load_demographic_data(
        ...
        if file_path:
            ...
            if not file.exists():
                raise FileNotFoundError(
                    f"Provided demographic file does not exist: {file}"
                )
        else:
    ...
    def get_summary_stats(self):
        ...
        if self.enrollment_data is not None:
            stats["enrollment"] = {
                ...
                "date_range": (
                    f"{self.enrollment_data['date'].min()} to {self.enrollment_data['date'].max()}"
                    if "date" in self.enrollment_data.columns
                    else None
                ),
                "states": (
                    self.enrollment_data["state"].nunique()
                    if "state" in self.enrollment_data.columns
                    else None
                ),
                "districts": (
                    self.enrollment_data["district"].nunique()
                    if "district" in self.enrollment_data.columns
                    else None
                ),
        ...
        if self.biometric_data is not None:
            stats["biometric"] = {
                ...
                "date_range": (
                    f"{self.biometric_data['date'].min()} to {self.biometric_data['date'].max()}"
                    if "date" in self.biometric_data.columns
                    else None
                ),
                "states": (
                    self.biometric_data["state"].nunique()
                    if "state" in self.biometric_data.columns
                    else None
                ),
                "districts": (
                    self.biometric_data["district"].nunique()
                    if "district" in self.biometric_data.columns
                    else None
                ),
        ...
        if self.demographic_data is not None:
            stats["demographic"] = {
                ...
                "date_range": (
                    f"{self.demographic_data['date'].min()} to {self.demographic_data['date'].max()}"
                    if "date" in self.demographic_data.columns
                    else None
                ),
                "states": (
                    self.demographic_data["state"].nunique()
                    if "state" in self.demographic_data.columns
                    else None
                ),
                "districts": (
                    self.demographic_data["district"].nunique()
                    if "district" in self.demographic_data.columns
                    else None
                ),
...
def _main():
    ...
    parser.add_argument(
        "--sample",
        default="500000",
        help="Sample size token to match in filenames (default: 500000)",
    )
