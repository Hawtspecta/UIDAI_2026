...
class UFIInsightGenerator:
    ...
    def identify_update_deserts(self, threshold=10):
        ...
        deserts = self.ufi_data[
            (self.ufi_data['total_enrollments'] > self.ufi_data['total_enrollments'].quantile(0.5)) &
            (self.ufi_data['UFI'] < threshold)
        ].sort_values('total_enrollments', ascending=False)
        
        self.insights['update_deserts'] = {
            'count': len(deserts),
            'top_10': deserts.head(10)[['state', 'district', 'UFI', 'total_enrollments']].to_dict('records'),
    ...
    def identify_high_friction_zones(self, threshold=75):
        ...
        high_friction = self.ufi_data[
            self.ufi_data['UFI'] > threshold
        ].sort_values('UFI', ascending=False)
        
        self.insights['high_friction_zones'] = {
            'count': len(high_friction),
            'top_10': high_friction.head(10)[['state', 'district', 'UFI', 'UFI_Category']].to_dict('records'),
    ...
    def analyze_state_level_patterns(self):
        ...
        state_summary = self.ufi_data.groupby('state').agg({
            'UFI': ['mean', 'median', 'std'],
            'district': 'count',
            'total_enrollments': 'sum'
        }).round(2)
        
        state_summary.columns = ['UFI_Mean', 'UFI_Median', 'UFI_StdDev', 'Num_Districts', 'Total_Enrollments']
        state_summary = state_summary.sort_values('UFI_Mean', ascending=False).reset_index()
    ...
    def calculate_impact_metrics(self):
        ...
        print(f"   ✅ System health: {low_friction_pct:.1f}% low friction, {high_friction_pct:.1f}% high friction")
    ...
    def export_insights(self, output_dir='outputs/reports'):
        ...
        with open(txt_path, 'w') as f:
            ...
            f.write(f"Total Districts Analyzed: {metrics.get('total_districts_analyzed', 'N/A')}\n")
            f.write(f"Mean UFI Score: {metrics.get('mean_ufi', 'N/A'):.2f}\n")
            f.write(f"Districts Needing Intervention: {metrics.get('districts_needing_intervention', 'N/A')}\n\n")
...
if __name__ == "__main__":
    ...
    print(f"2. HIGH FRICTION ZONES: {insights['high_friction_zones']['count']} districts experiencing severe stress")
    print(f"3. SYSTEM HEALTH: {insights['impact_metrics']['low_friction_percentage']:.1f}% districts stable")
    print(f"4. INTERVENTION NEEDED: {insights['impact_metrics']['districts_needing_intervention']} districts")