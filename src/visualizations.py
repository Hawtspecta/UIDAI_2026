...
class UFIVisualizer:
    ...
    def plot_ufi_distribution(self):
        ...
        ax1.axvline(
            self.ufi_data["UFI"].mean(),
            color="red",
            linestyle="--",
            label=f'Mean: {self.ufi_data["UFI"].mean():.2f}',
        )
        ax1.axvline(
            self.ufi_data["UFI"].median(),
            color="green",
            linestyle="--",
            label=f'Median: {self.ufi_data["UFI"].median():.2f}',
        )
        ...
        self.ufi_data["UFI_Category"] = pd.Categorical(
            self.ufi_data["UFI_Category"], categories=category_order, ordered=True
        )

        sns.boxplot(
            data=self.ufi_data, x="UFI_Category", y="UFI", ax=ax2, palette="RdYlGn_r"
        )
    ...
    def plot_top_states_ufi(self, top_n=15):
        ...
        state_ufi = (
            self.ufi_data.groupby("state")["UFI"]
            .mean()
    ...
    def plot_component_contributions(self):
        ...
        ax.set_title(
            "UFI Component Breakdown - Top 10 High Friction Districts",
            fontsize=14,
            fontweight="bold",
        )
    ...
    def plot_scatter_matrix(self):
        ...
        for ax, (col, title) in zip(axes.flat, components):
            ...
            p = np.poly1d(z)
            ax.plot(
                self.ufi_data[col], p(self.ufi_data[col]), "r--", alpha=0.8, linewidth=2
            )
    ...
    def plot_category_distribution(self):
        ...
        ax.set_title(
            "District Distribution by UFI Category", fontsize=14, fontweight="bold"
        )
